...
namespace AddUp.CommonLogging.Factory
{
    ...
    public class AbstractLoggerTests
    {
        ...
        [Test]
        public void IsSerializable() => Assert.IsTrue(SerializationTestUtils.IsSerializable<AbstractLogger>());
        ...
        public void ImplementsAllMethodsForAllLevels()
        {
            ...
            foreach (string logLevel in logLevels)
            {
                ...
                for (var i = 0; i < logLevels.Length; i++)
                    Assert.IsNotNull(logMethods[i], "Method with signature #{0} not implemented for level {1}", i, logLevel);
            }
        }
